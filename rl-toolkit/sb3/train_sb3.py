...
parser.add_argument("--n-timesteps", type=int, default=int(1e4),
                    help="Number of time steps for trainng")
parser.add_argument("--selected", action='store_true')
parser.add_argument("--n-eval-episodes", default=10, type=int, help="Number of evaluation episodes to test agent")
parser.add_argument("--save", action='store_true', help="Whether to save model")
parser.add_argument("--output-name", type=str, default="trash", help="What file name to use for all outputs")
parser.add_argument("--plot", action='store_true', help="Choose to ouput a png plot of SAR")
...
def main():
    # In the first if then block, fetching hyperparameters either from a yaml in
    # the hyperparams directory or from the best trial in the specified study.
    if args.selected:
        assert os.path.exists("hyperparams"), "No hyperparams directory found."
        assert os.path.exists(f"hyperparams/{args.env}.yaml"), "No yaml file found for this environment."
        ...
        if "algorithm" in study.system_attrs:
            assert args.algorithm == study.system_attrs["algorithm"], "Algorithm used in study and for training are not the same"
    ...
    model = algo_utils[args.algorithm]("MlpPolicy", env, verbose=2,
